...
#ifdef HAVE_OPCACHE_REDIS_CACHE

#include "ZendAccelerator.h"
#include "zend_redis_cache.h"
...
#define SERIALIZE_STR(ptr) do { \
		if (ptr) { \
			if (IS_ACCEL_INTERNED(ptr)) { \
				(ptr) = zend_redis_cache_serialize_interned((zend_string*)(ptr), info); \
			} else { \
				ZEND_ASSERT(IS_UNSERIALIZED(ptr)); \
...
#define UNSERIALIZE_STR(ptr) do { \
		if (ptr) { \
			if (IS_SERIALIZED_INTERNED(ptr)) { \
				(ptr) = (void*)zend_redis_cache_unserialize_interned((zend_string*)(ptr), !script->corrupted); \
			} else { \
				ZEND_ASSERT(IS_SERIALIZED(ptr)); \
				(ptr) = (void*)((char*)buf + (size_t)(ptr)); \
				/* script->corrupted shows if the script in SHM or not */ \
...
typedef struct _zend_redis_cache_metainfo {
	char         magic[8];
	char         system_id[32];
	size_t       mem_size;
	size_t       str_size;
	...
	uint32_t     checksum;
} zend_redis_cache_metainfo;


typedef void (*serialize_callback_t)(zval                     *zv,
                                     zend_persistent_script   *script,
                                     zend_redis_cache_metainfo *info,
                                     void                     *buf);
...
static void zend_redis_cache_serialize_zval(zval                     *zv,
                                           zend_persistent_script   *script,
                                           zend_redis_cache_metainfo *info,
                                           void                     *buf);
static void zend_redis_cache_unserialize_zval(zval                    *zv,
                                             zend_persistent_script  *script,
                                             void                    *buf);

static void *zend_redis_cache_serialize_interned(zend_string              *str,
                                                zend_redis_cache_metainfo *info)
{
	size_t len;
	void *ret;
...
static void *zend_redis_cache_unserialize_interned(zend_string *str, int in_shm)
{
	zend_string *ret;
...
static void zend_redis_cache_serialize_hash(HashTable                *ht,
                                           zend_persistent_script   *script,
                                           zend_redis_cache_metainfo *info,
                                           void                     *buf,
...
static zend_ast *zend_redis_cache_serialize_ast(zend_ast                 *ast,
                                               zend_persistent_script   *script,
                                               zend_redis_cache_metainfo *info,
                                               void                     *buf)
{
	...
	if (ast->kind == ZEND_AST_ZVAL) {
		zend_redis_cache_serialize_zval(&((zend_ast_zval*)ast)->val, script, info, buf);
	} else if (zend_ast_is_list(ast)) {
		zend_ast_list *list = zend_ast_get_list(ast);
		for (i = 0; i < list->children; i++) {
			if (list->child[i]) {
				list->child[i] = zend_redis_cache_serialize_ast(list->child[i], script, info, buf);
			}
		}
	} else {
		...
		for (i = 0; i < children; i++) {
			if (ast->child[i]) {
				ast->child[i] = zend_redis_cache_serialize_ast(ast->child[i], script, info, buf);
			}
		}
	}
	return ret;
	...
	switch (Z_TYPE_P(zv)) {
		...
		case IS_ARRAY:
			if (!IS_SERIALIZED(Z_ARR_P(zv))) {
				...
				ht = Z_ARR_P(zv);
				UNSERIALIZE_PTR(ht);
				zend_redis_cache_serialize_hash(ht, script, info, buf, zend_redis_cache_serialize_zval);
			}
			break;
		case IS_REFERENCE:
			if (!IS_SERIALIZED(Z_REF_P(zv))) {
				...
				ref = Z_REF_P(zv);
				UNSERIALIZE_PTR(ref);
				zend_redis_cache_serialize_zval(&ref->val, script, info, buf);
			}
			break;
		case IS_CONSTANT_AST:
			if (!IS_SERIALIZED(Z_AST_P(zv))) {
				...
				if (!IS_SERIALIZED(ast->ast)) {
					ast->ast = zend_redis_cache_serialize_ast(ast->ast, script, info, buf);
				}
			}
			break;
	}
...
static void zend_redis_cache_serialize_op_array(zend_op_array            *op_array,
                                               zend_persistent_script   *script,
                                               zend_redis_cache_metainfo *info,
                                               void                     *buf)
{
	...
	if (op_array->literals && !IS_SERIALIZED(op_array->literals)) {
		...
		while (p < end) {
			zend_redis_cache_serialize_zval(p, script, info, buf);
			p++;
		}
	}
...
static void zend_redis_cache_serialize_func(zval                     *zv,
                                           zend_persistent_script   *script,
                                           zend_redis_cache_metainfo *info,
                                           void                     *buf)
{
	...
	zend_redis_cache_serialize_op_array(op_array, script, info, buf);
}

static void zend_redis_cache_serialize_prop_info(zval                     *zv,
                                                zend_persistent_script   *script,
                                                zend_redis_cache_metainfo *info,
                                                void                     *buf)
{
...
static void zend_redis_cache_serialize_class(zval                     *zv,
                                            zend_persistent_script   *script,
                                            zend_redis_cache_metainfo *info,
                                            void                     *buf)
{
	...
	zend_redis_cache_serialize_hash(&ce->function_table, script, info, buf, zend_redis_cache_serialize_func);
	...
	zend_redis_cache_serialize_hash(&ce->constants_table, script, info, buf, zend_redis_cache_serialize_zval);
	SERIALIZE_STR(ZEND_CE_FILENAME(ce));
	SERIALIZE_STR(ZEND_CE_DOC_COMMENT(ce));
	zend_redis_cache_serialize_hash(&ce->properties_info, script, info, buf, zend_redis_cache_serialize_prop_info);
...
static void zend_redis_cache_serialize(zend_persistent_script   *script,
                                      zend_redis_cache_metainfo *info,
                                      void                     *buf)
{
	...
	zend_redis_cache_serialize_hash(&new_script->class_table, script, info, buf, zend_redis_cache_serialize_class);
	zend_redis_cache_serialize_hash(&new_script->function_table, script, info, buf, zend_redis_cache_serialize_func);
	zend_redis_cache_serialize_op_array(&new_script->main_op_array, script, info, buf);

	SERIALIZE_PTR(new_script->arena_mem);
	new_script->mem = NULL;
}

int zend_redis_cache_script_store(zend_persistent_script *script, int in_shm)
{
	redisContext *c;
	const char *hostname = ZCG(accel_directives).redis_cache;
	char *filename;
	filename = ZSTR_VAL(script->full_path);
	int port = 6379;
	...
	zend_redis_cache_serialize(script, &info, buf);
	if (!in_shm) {
		script->corrupted = 0;
	}
	...
	redisCommand(c,"SET %s %b%b%b", filename, &info, sizeof(info), buf, script->size, ZSTR_VAL((zend_string*)ZCG(mem)), info.str_size);
...
static void zend_redis_cache_unserialize_hash(HashTable               *ht,
                                             zend_persistent_script  *script,
                                             void                    *buf,
...
static zend_ast *zend_redis_cache_unserialize_ast(zend_ast                *ast,
                                                 zend_persistent_script  *script,
                                                 void                    *buf)
{
	uint32_t i;
	...
	if (ast->kind == ZEND_AST_ZVAL) {
		zend_redis_cache_unserialize_zval(&((zend_ast_zval*)ast)->val, script, buf);
	} else if (zend_ast_is_list(ast)) {
		zend_ast_list *list = zend_ast_get_list(ast);
		for (i = 0; i < list->children; i++) {
			if (list->child[i]) {
				list->child[i] = zend_redis_cache_unserialize_ast(list->child[i], script, buf);
			}
		}
	} else {
		uint32_t children = zend_ast_get_num_children(ast);
		for (i = 0; i < children; i++) {
			if (ast->child[i]) {
				ast->child[i] = zend_redis_cache_unserialize_ast(ast->child[i], script, buf);
			}
		}
	}
	return ast;
	...
	switch (Z_TYPE_P(zv)) {
		...
		case IS_ARRAY:
			if (!IS_UNSERIALIZED(Z_ARR_P(zv))) {
				...
				ht = Z_ARR_P(zv);
				zend_redis_cache_unserialize_hash(ht,
						script, buf, zend_redis_cache_unserialize_zval, ZVAL_PTR_DTOR);
			}
			break;
		case IS_REFERENCE:
			if (!IS_UNSERIALIZED(Z_REF_P(zv))) {
				...
				ref = Z_REF_P(zv);
				zend_redis_cache_unserialize_zval(&ref->val, script, buf);
			}
			break;
		case IS_CONSTANT_AST:
			if (!IS_UNSERIALIZED(Z_AST_P(zv))) {
				...
				if (!IS_UNSERIALIZED(ast->ast)) {
					ast->ast = zend_redis_cache_unserialize_ast(ast->ast, script, buf);
				}
			}
			break;
	}
...
static void zend_redis_cache_unserialize_op_array(zend_op_array           *op_array,
                                                 zend_persistent_script  *script,
                                                 void                    *buf)
{
	...
	if (op_array->literals && !IS_UNSERIALIZED(op_array->literals)) {
		...
		while (p < end) {
			zend_redis_cache_unserialize_zval(p, script, buf);
			p++;
		}
	}
...
static void zend_redis_cache_unserialize_func(zval                    *zv,
                                             zend_persistent_script  *script,
                                             void                    *buf)
{
	zend_op_array *op_array;
	...
	op_array = Z_PTR_P(zv);
	zend_redis_cache_unserialize_op_array(op_array, script, buf);
}

static void zend_redis_cache_unserialize_prop_info(zval                    *zv,
                                                  zend_persistent_script  *script,
                                                  void                    *buf)
{
...
static void zend_redis_cache_unserialize_class(zval                    *zv,
                                              zend_persistent_script  *script,
                                              void                    *buf)
{
	zend_class_entry *ce;
	...
	zend_redis_cache_unserialize_hash(&ce->function_table,
			script, buf, zend_redis_cache_unserialize_func, ZEND_FUNCTION_DTOR);
	if (ce->default_properties_table) {
		zval *p, *end;
	...
	zend_redis_cache_unserialize_hash(&ce->constants_table,
			script, buf, zend_redis_cache_unserialize_zval, NULL);
	UNSERIALIZE_STR(ZEND_CE_FILENAME(ce));
	UNSERIALIZE_STR(ZEND_CE_DOC_COMMENT(ce));
	zend_redis_cache_unserialize_hash(&ce->properties_info,
			script, buf, zend_redis_cache_unserialize_prop_info, ZVAL_PTR_DTOR);
...
static void zend_redis_cache_unserialize(zend_persistent_script  *script,
                                        void                    *buf)
{
	script->mem = buf;
	...
	zend_redis_cache_unserialize_hash(&script->class_table,
			script, buf, zend_redis_cache_unserialize_class, ZEND_CLASS_DTOR);
	zend_redis_cache_unserialize_hash(&script->function_table,
			script, buf, zend_redis_cache_unserialize_func, ZEND_FUNCTION_DTOR);
	zend_redis_cache_unserialize_op_array(&script->main_op_array, script, buf);

	UNSERIALIZE_PTR(script->arena_mem);
}

zend_persistent_script *zend_redis_cache_script_load(zend_file_handle *file_handle)
{
	char *filename;
    ...
    if (c == NULL || c->err) {
        if (c) {
        	zend_accel_error(ACCEL_LOG_WARNING, "opcache cannot connect to redis: '%s'\n", c->errstr);
            redisFree(c);
        } else {
        	zend_accel_error(ACCEL_LOG_WARNING, "opcache cannot allocate redis context\n");
        }
        return NULL;
    }

    reply = redisCommand(c,"GET %s", filename);

    if (reply->len > sizeof(info)) {
    	memcpy(&info, reply->str, sizeof(info));
    	/*if (reply->len + sizeof(info) != info.mem_size + info.str_size) {
	...
	if (ZCG(accel_directives).validate_timestamps &&
	    zend_get_file_handle_timestamp(file_handle, NULL) > info.timestamp + ZCG(accel_directives).revalidate_freq) {
		freeReplyObject(reply);
		return NULL;
	}
	...
	memcpy(mem, &reply->str[sizeof(info)], info.mem_size + info.str_size);
...
use_process_mem:
	...
	zend_redis_cache_unserialize(script, buf);
	script->corrupted = 0;
...
void zend_redis_cache_invalidate(zend_string *full_path)
{
	redisContext *c;
	const char *hostname = ZCG(accel_directives).redis_cache;
	int port = 6379;
	...
	redisCommand(c,"DEL %s", ZSTR_VAL(full_path));
}

#endif /* HAVE_OPCACHE_REDIS_CACHE */
